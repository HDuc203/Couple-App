...
function ProfileSection({ profile }: { profile: Profile }) {
  ...
  // If gender is male/not female, disable and force uncheck period tracking
  useEffect(() => {
    if (gender !== "female") {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="space-y-2">
        <label className={`flex items-center gap-3 rounded-2xl bg-[var(--color-surface)] border border-[var(--color-border)] px-4 py-3 text-sm font-bold text-[var(--color-text)] cursor-pointer ${gender !== "female" ? "opacity-50 cursor-not-allowed" : ""}`}>
          <input
            type="checkbox"
