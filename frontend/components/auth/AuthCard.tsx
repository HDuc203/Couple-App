...
export function AuthCard({
  ...
  const handleForgotPasswordSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (res && res.success) {
        ...
        setResetEmail(email || "");
        setClientMessage(res.message || "Yêu cầu khôi phục đã được gửi.");
      } else if (res) {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      setClientError(err.message || "Không thể khởi động đăng nhập Google.");
    } finally {
      setIsPending(false);
    }
  };
        ...
        <section className="auth-forms-panel bg-transparent lg:bg-white/5 p-6 sm:p-8 lg:p-8 xl:p-10 flex flex-col justify-between items-center relative overflow-hidden h-full">
          ...
          <div className="w-full flex items-center justify-center gap-6 py-2 relative">
            ...
            >
              ...
              {isVerifyingEmail ? (
                ...
                isResetEmailSent ? (
                  <div className="space-y-6 py-4 text-center animate-fade-in">
                    ...
                    <div className="space-y-2">
                      <h3 className="text-lg font-black text-[#5a4c35] tracking-tight">Kiểm tra hộp thư</h3>
                      <p className="text-xs font-semibold text-[#7c6a50] leading-relaxed px-2">
                      ...
                      <p className="text-[11px] font-semibold text-[#8c7e6b] leading-relaxed px-4 pt-1">
                        Vui lòng nhấp vào liên kết trong email để đặt lại mật khẩu của bạn.
                      </p>
                    </div>
                  ...
                  <form onSubmit={handleForgotPasswordSubmit} className="space-y-4 animate-fade-in">
                    <div className="text-center py-1 space-y-1">
                      <h3 className="text-sm font-black text-[#5a4c35]">Khôi phục mật khẩu</h3>
                      <p className="text-[11px] font-semibold text-[#8c7e6b] leading-relaxed px-4">
                        Nhập email liên kết với tài khoản của bạn để nhận liên kết khôi phục mật khẩu.
                      </p>
                    </div>
                    ...
                    <div className="pt-2 text-center border-t border-[#ebdcb9]/20">
                      <button
                        type="button"
                        onClick={() => {
                          setIsForgotPassword(false);
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="text-right">
                          <button
                            ...
                            className="text-[10px] font-bold text-[#c4a15a] hover:underline bg-transparent border-none p-0 cursor-pointer"
                          >
                            Quên mật khẩu?
                          </button>
                        </div>
              ...
              {isLogin && !isVerifyingEmail && !isForgotPassword && (
                <div className="mt-4 space-y-3">
                  ...
                  <button
                    ...
                    disabled={isPending}
                    className="w-full h-10 flex items-center justify-center gap-2 rounded-2xl border border-[#ebdcb9]/40 bg-white hover:bg-zinc-50 text-xs font-black text-zinc-600 transition active:scale-[0.99] disabled:opacity-50"
                  ...
                  >
                    ...
                    <span>{isPending ? "Đang xử lý..." : "Tiếp tục với Google"}</span>
                  </button>
                </div>
              )}

              {!isVerifyingEmail && !isForgotPassword && (
