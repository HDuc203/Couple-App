...
export function FutureSpace({
  ...
  const handleToggleComplete = async (item: Tables<"bucket_list">) => {
    if (item.is_completed) return; // Prevent completing already completed items
    ...
    if (error) {
      ...
      setItems((prev) =>
        prev.map((i) =>
          i.id === item.id
            ? { ...i, is_completed: nextStatus, completed_at: new Date().toISOString() }
            : i
        )
      );
      ...
      setCompletedDream({ ...item, is_completed: nextStatus, completed_at: new Date().toISOString() });
      startTransition(() => {
        router.refresh();
      });
    }
